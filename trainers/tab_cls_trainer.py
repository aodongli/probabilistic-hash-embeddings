...
class DyTabBinCls_Trainer:
    ...
    def train_scheduler(self, db, dataset_class, tot_task=3):
        ...
        for t in range(tot_task):
            ...
            val = dataset_class(dat_val[:, :-1], dat_val[:, -1], db.incre_col_idx, db.cat_id)
    ...
    def global_train_scheduler(self, db, dataset_class, tot_task=3):
        ...
        tr = dataset_class(tot_dat_tr[:, :-1], tot_dat_tr[:, -1], db.incre_col_idx, db.cat_id)
        val = dataset_class(tot_dat_val[:, :-1], tot_dat_val[:, -1], db.incre_col_idx, db.cat_id)
        te = dataset_class(tot_dat_te[:, :-1], tot_dat_te[:, -1], db.incre_col_idx, db.cat_id)
    ...
    def test(self, test_dataset, ckpt_path=''):
        ...
        return [str(acc), str(np.mean(prob_stds)), str(np.mean(nelbos))]