...
class OnlineMushroomData:
    ...
    def get_all_task_info(self):
        ...
        self.subsequent_task_size = 3 #5 #100
    ...
    def get_dataset(self, task_id=0):
        ...
        if self.env_config.random_task_size:
            resize = self.rng.randint(min(5, self.subsequent_task_size), self.subsequent_task_size+1)
            data = data[:resize]
        if self.env_config.noise_obs:
...
def main():
    ...
    db = OnlineMushroomData('../data/tabular_data/mushroom/', env_config=env_config)
    df = db.df
    print(df)
