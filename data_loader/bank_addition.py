...
class ContinualBankAddData:
    def __init__(self, root, model_config=None, env_config=None):
        ...
        df = pd.read_csv(self.root + 'bank-additional/bank-additional-full.csv', header=0, sep=';', engine='python')
        
        # convert labels
        df['y'] = df['y'].replace({'no':0, 'yes':1})
        
        # normalize
        for col in ['age', 'duration', 'campaign', 'pdays', 'previous', 'emp.var.rate', 'cons.price.idx', 'cons.conf.idx', 'euribor3m', 'nr.employed']:
        ...
        self.columns = ['age', 'duration', 'campaign', 'pdays', 'previous', 'emp.var.rate', 'cons.price.idx', 'cons.conf.idx', 'euribor3m', 'nr.employed', "job", "marital", "education", "default", "housing", "loan", "contact", "month", "day_of_week", "poutcome",  'y']
...
def main():
    ...
    df.drop(['job', 'education', 'marital', 'default', 'housing', 'loan', 'contact', 'month', 'day_of_week', 'poutcome'],
            axis=1, 
            inplace=True)
    df = pd.concat([df, job, education, marital, default, housing, loan, contact, month, day_of_week, poutcome],
                   axis=1)
    print(df.columns)
